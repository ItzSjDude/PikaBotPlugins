...
async def helper(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@"):
        ...
        help_string = f"""IɴFɪNɪX {(await infchvar(event,"helpstr",6)).message.format(web=_web,ghub=_gt)}"""
...
if pdb.Omega is not None and tgbot is not None:
    ...
    async def _infxcallback(infx_):
        _infxa_ = (infx_.query).user_id
        if _infxa_ in Ccache:
            _infx = f"""IɴFɪNɪX {(await infchvar(infx_,"helpstr",6)).message.format(web=_web,ghub=_gt)}"""
            _infxcmds = bot.infx_cmd
